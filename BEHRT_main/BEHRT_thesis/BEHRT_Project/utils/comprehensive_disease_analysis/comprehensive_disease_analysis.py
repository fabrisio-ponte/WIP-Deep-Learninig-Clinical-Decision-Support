...
class ComprehensiveDiseaseAnalysis:
    ...
    def analyze_individual_diseases(self, top_n: int = 10):
        ...
        for disease_idx in range(len(self.label_vocab)):
            ...
            disease_probabilities = self.probabilities[:, disease_idx]
            ...
            # Per-disease threshold-free ranking metrics for reproducible table generation.
            positives = int(np.sum(disease_labels))
            negatives = int(len(disease_labels) - positives)
            if positives == 0:
                ...
                average_precision = average_precision_score(disease_labels, disease_probabilities)
                roc_auc = roc_auc_score(disease_labels, disease_probabilities)
    ...
    def save_results(self, output_file: str = "comprehensive_disease_analysis_results.json"):
        """Save detailed results to JSON file and per-disease CSV artifact."""
        ...
        # Also save a flat CSV artifact for manuscript tables and external QA.
        per_disease_df = pd.DataFrame(self.individual_disease_metrics)
        csv_path = Path(output_file).with_name(Path(output_file).stem + "_per_disease_metrics.csv")
        per_disease_df.to_csv(csv_path, index=False)
        print(f"✓ Per-disease metrics saved to {csv_path}")
