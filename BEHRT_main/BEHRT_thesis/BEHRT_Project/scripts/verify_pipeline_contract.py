...
def get_checkpoint_classifier_dim(checkpoint_path: Path):
...
def resolve_checkpoint_path(project_root: Path, config: dict, expected_vocab_size: int | None = None):
    configured = project_root / config.get("model_path", "") if config.get("model_path") else None
    candidates = []
    if configured is not None and configured.exists():
        candidates.append(configured)

    clean_runs_dir = project_root / "data" / "models" / "clean_runs"
    if clean_runs_dir.exists():
        for run_dir in sorted(clean_runs_dir.glob("clean_run_*"), reverse=True):
            candidate = run_dir / "behrt_nextvisit_ccsr_clean_best.pt"
            if candidate.exists():
    ...
    for candidate in candidates:
        ...
        dim = get_checkpoint_classifier_dim(candidate)
        if expected_vocab_size is not None:
            if dim == expected_vocab_size:
                return candidate
        else:
            if dim is not None:
                return candidate
...
def main():
    ...
    expected_vocab_size = len(format_label_vocab(vocab["token2idx"])) if vocab_exists else None
    checkpoint_path = resolve_checkpoint_path(project_root, config, expected_vocab_size)
    ckpt_exists = checkpoint_path is not None and checkpoint_path.exists()
    checks.append(("checkpoint_exists", ckpt_exists, str(checkpoint_path) if checkpoint_path is not None else "<no checkpoint found>"))

    if ckpt_exists:
        classifier_weight = None
        classifier_dim = get_checkpoint_classifier_dim(checkpoint_path)
        if classifier_dim is not None:
            classifier_weight = classifier_dim

        if classifier_weight is not None:
            out_features = int(classifier_weight)
