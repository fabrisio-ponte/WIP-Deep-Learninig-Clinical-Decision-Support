"""
MIMIC-IV to BEHRT Format Converter with CCSR Mapping
This script processes MIMIC-IV data and converts it to BEHRT format using CCSR categories.
"""
...
CCS_MAPPING_PATH = "../cssr_mappings/ccs_mapping_unified_official.pkl"  # Your new CCS mapping
...
if len(sys.argv) > 2:
    CCS_MAPPING_PATH = sys.argv[2]

# Create output directory
os.makedirs(OUTPUT_PATH, exist_ok=True)

def load_ccs_mapping():
    """Load the unified CCSR mapping"""
    print("Loading CCSR mapping...")
    try:
        with open(CCS_MAPPING_PATH, 'rb') as f:
            ccs_mapping = pickle.load(f)
        print(f"✓ Loaded CCSR mapping with {len(ccs_mapping)} codes")
        return ccs_mapping
    except FileNotFoundError:
        print(f" CCSR mapping file not found: {CCS_MAPPING_PATH}")
        print("Please run create_complete_mapping.py first to generate the mapping")
        sys.exit(1)
...
def map_to_ccsr(diagnoses, ccs_mapping):
    """Map ICD codes to CCSR categories"""
    print("Mapping ICD codes to CCSR categories...")
    
    # Create clean ICD codes (remove dots and handle versions)
    diagnoses['icd_clean'] = diagnoses['icd_code'].str.replace('.', '', regex=False)
    
    # Map ICD-9 codes (version 9)
    icd9_mask = diagnoses['icd_version'] == 9
    diagnoses.loc[icd9_mask, 'ccsr_code'] = diagnoses.loc[icd9_mask, 'icd_clean'].map(ccs_mapping)
    
    # Map ICD-10 codes (version 10)  
    icd10_mask = diagnoses['icd_version'] == 10
    diagnoses.loc[icd10_mask, 'ccsr_code'] = diagnoses.loc[icd10_mask, 'icd_clean'].map(ccs_mapping)
    
    # Statistics
    mapped_count = diagnoses['ccsr_code'].notna().sum()
    unmapped_count = diagnoses['ccsr_code'].isna().sum()
    
    print(f"✓ Mapped {mapped_count:,} codes to CCSR ({unmapped_count:,} unmapped)")
    print(f"  Unique CCSR categories: {diagnoses['ccsr_code'].nunique()}")
    
    # Show some mapping examples
    print("\nSample ICD → CCSR mappings:")
    sample_mappings = diagnoses[diagnoses['ccsr_code'].notna()].head(5)
    for _, row in sample_mappings.iterrows():
        print(f"  ICD-{row['icd_version']}: {row['icd_code']:10} → {row['ccsr_code']}")
    
    # Remove unmapped codes
    diagnoses_mapped = diagnoses[diagnoses['ccsr_code'].notna()].copy()
    print(f"✓ Keeping {len(diagnoses_mapped):,} mapped diagnoses")
...
def merge_and_format(admissions_with_age, diagnoses_ccsr):
    """Merge CCSR diagnoses with admissions and format"""
    print("Merging CCSR diagnoses with admissions...")
    
    # Merge CCSR diagnoses with admissions
    df = diagnoses_ccsr.merge(admissions_with_age, on=['subject_id', 'hadm_id'], how='inner')
    
    # Use CCSR code as the main code
    df['code'] = 'CCSR_' + df['ccsr_code'].astype(str)
    ...
    print(f"Total CCSR records: {len(df)}")
    print(f"Unique CCSR categories in data: {df['code'].nunique()}")
    return df
...
def create_visit_sequences(df):
    ...
    for subject_id, group in df.groupby('subject_id'):
        ...
        for hadm_id, visit in group.groupby('hadm_id'):
            ...
            # Get unique CCSR codes for this visit (remove duplicates within visit)
            visit_codes = visit['code'].unique().tolist()
            visit_ages = visit['age_months'].tolist()
            
            # Add codes and ages
            codes.extend(visit_codes)
            ages.extend([visit_ages[0]] * len(visit_codes))  # Same age for all codes in visit
            
            # Add SEP token between visits (except after last visit)
            if visit_count < len(group['hadm_id'].unique()):
                codes.append('SEP')
                ages.append(visit_ages[0])
...
def create_vocabulary(sequences_df):
    """Create vocabulary from all CCSR codes"""
    ...
    print(f"CCSR categories in vocabulary: {len(sorted_codes)}")
    return vocab
...
def save_data(train_df, val_df, test_df, vocab, age_vocab):
    ...
    # Save dataframes
    train_df.to_parquet(f"{OUTPUT_PATH}/train_mlm_ccsr.parquet", index=False)
    val_df.to_parquet(f"{OUTPUT_PATH}/val_mlm_ccsr.parquet", index=False)
    test_df.to_parquet(f"{OUTPUT_PATH}/test_mlm_ccsr.parquet", index=False)
    
    # Save vocabularies
    with open(f"{OUTPUT_PATH}/vocab_ccsr.pkl", 'wb') as f:
        pickle.dump(vocab, f)
    ...
    print("\nFiles created:")
    print("  - train_mlm_ccsr.parquet: Training data for MLM with CCSR")
    print("  - val_mlm_ccsr.parquet: Validation data for MLM with CCSR")
    print("  - test_mlm_ccsr.parquet: Test data for MLM with CCSR")
    print("  - vocab_ccsr.pkl: CCSR vocabulary")
    print("  - age_vocab.pkl: Age vocabulary")

def main():
    """Main processing pipeline with CCSR mapping"""
    print("="*60)
    print("MIMIC-IV to BEHRT Format Converter with CCSR Mapping")
    print("="*60)
    
    # Step 1: Load CCSR mapping
    ccs_mapping = load_ccs_mapping()
    
    # Step 2: Load MIMIC data
    patients, admissions, diagnoses = load_mimic_data()
    
    # Step 3: Map ICD codes to CCSR
    diagnoses_ccsr = map_to_ccsr(diagnoses, ccs_mapping)
    
    # Step 4: Calculate ages
    admissions_with_age = calculate_age(patients, admissions)
    
    # Step 5: Merge and format with CCSR
    formatted_df = merge_and_format(admissions_with_age, diagnoses_ccsr)
    
    # Step 6: Create sequences
    sequences_df = create_visit_sequences(formatted_df)
    
    # Step 7: Create vocabularies
    ...
    # Save labeled data
    train_labeled.to_parquet(f"{OUTPUT_PATH}/train_nextvisit_ccsr.parquet", index=False)
    val_labeled.to_parquet(f"{OUTPUT_PATH}/val_nextvisit_ccsr.parquet", index=False)
    test_labeled.to_parquet(f"{OUTPUT_PATH}/test_nextvisit_ccsr.parquet", index=False)
    ...
    print(f"  CCSR vocabulary size: {len(vocab['token2idx'])}")
    ...
    print("\nBenefits of CCSR mapping:")
    print("  ✓ Clinical grouping of similar conditions")
    print("  ✓ Reduced dimensionality (89,000+ codes → 495 categories)")
    print("  ✓ Consistent across ICD-9 and ICD-10")
    print("  ✓ More meaningful clinical representations")
    
    print("\nNext steps:")
    print("  1. Run MLM pre-training using MLM.ipynb with CCSR data")
